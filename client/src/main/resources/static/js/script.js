...
const connect = (event) => {
    ...
    if (username) {
        ...
        const endpoint = '/chat-example'

        const socket = new SockJS(url+endpoint)
...
const onConnected = () => {
    ...
    stompClient.send("/app/chat.newUser",
        {},
        JSON.stringify({sender: username, type: 'CONNECT', timestamp: moment().calendar()})
    )
...
const onMessageReceived = (payload) => {
    ...
    if (message.type === 'CONNECT') {
        ...
        messageElement.classList.add('event-message')
        message.content = '<b>' + message.sender + '</b> left!'
    } else {
        ...
        const avatarText = document.createTextNode(getInitial(message.sender))
    ...
    messageElement.innerHTML = message.content + "<br>" + message.timestamp
